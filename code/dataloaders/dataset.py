...
class LAHeart(Dataset):
    ...
    def __getitem__(self, idx):
        image_name = self.image_list[idx]
        h5f = h5py.File(self._base_dir + "/2018LA_Seg_Training/" + image_name + "/mri_norm2.h5", 'r')
        image = h5f['image'][:]
        label = h5f['label'][:]
        sample = {'image': image, 'label': label}
        if self.transform:
...
class WeakStrongAugment(object):
    ...
    def __call__(self, sample):
        ...
        image_strong = deepcopy(image_weak)
        # apply color aug
...
class DoubleStrongAugment(object):
    def __init__(self, output_size, p_color=0.8, p_blur=0.2, flag_rot=True):
        self.output_size = output_size
        self.randrotflip = RandomRotFlip()
        self.randcrop = RandomCrop(output_size)
    ...
    def __call__(self, sample):
        ...
        # strong aug view-1
        img_s1 = deepcopy(image_weak)
        # apply color aug
        img_s1 = self.randcolor(img_s1)
        # apply blur
        img_s1 = self.randblur(img_s1)
        
        # strong aug view-2
        img_s2 = deepcopy(image_weak)
        # apply color aug
        img_s2 = self.randcolor(img_s2)
        # apply blur
        img_s2 = self.randblur(img_s2)
    
        # to tensor
        img_s1 = torch.from_numpy(img_s1.astype(np.float32)).unsqueeze(0)
        img_s2 = torch.from_numpy(img_s2.astype(np.float32)).unsqueeze(0)
        image_weak = torch.from_numpy(image_weak.astype(np.float32)).unsqueeze(0)
        label = torch.from_numpy(label.astype(np.uint8)).long()
...
"""
Reference: https://github.com/ZhenZHAO/AD-MT/blob/main/code/dataloaders/dataset_3d.py
"""
