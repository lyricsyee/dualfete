...
"""
Reference:
**BCP** - https://github.com/DeepMed-Lab-ECNU/BCP/blob/main/code/utils/losses.py
"""
...
def onehot(label, n_cls):
    onehot_label = torch.zeros_like(label.unsqueeze(1))
    onehot_label = onehot_label.repeat(1, n_cls, 1, 1, 1)
...
class mask_DiceLoss(nn.Module):
    ...
    def prob_forward(self, pred, target, mask=None):
        ...
        # target_one_hot = onehot(target.type(torch.long), nclass).type(torch.float32)
    ...
    def forward(self, logits, target, mask=None):
        # size = logits.size()
        N, nclass = logits.size(0), logits.size(1)
...
class DiceLoss(nn.Module):
    ...
    def forward(self, inputs, target, weight=None, use_softmax=False):
        if use_softmax:
        ...
        n_cls = self.n_classes if self.ignore_index is None else self.n_classes / len(self.ignore_index)
        return loss / n_cls
